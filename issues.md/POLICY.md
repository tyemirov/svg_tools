...
- **Integration tests only**: Unit tests are prohibited. All tests MUST be black-box integration tests that exercise the real code path as it runs in production. Testing components in isolation creates false confidence—if the test doesn't run the actual deployed code path, it's worthless.
- We **strive for 100% test coverage** driven entirely by integration/black-box suites that exercise observable behavior through real entry points (HTTP handlers, CLI commands, public APIs), not mocked or isolated components.
...
- **Unit tests**: Testing components in isolation is forbidden. Tests MUST exercise the full code path as deployed.
